...
import { InvAgentModel } from "shared/models";

@Injectable({
  providedIn: "root"
})
export class InvAgentService {
  api_string = "/api/InvAgents";
  ...
  getAllRecords(query?): Observable<InvAgentModel[]> {
  ...
  postRecord(item?) {
    ...
    return this.http
      .post<InvAgentModel>(url_api, query, { headers: this.headers })
      .pipe(map(data => data));
  }
  ...
  putRecord(item?) {
    ...
    return this.http
      .put<InvAgentModel>(url_api, query, { headers: this.headers })
      .pipe(map(data => data));
  }
  ...
  deleteRecord(id) {
    const accessToken = localStorage.getItem("accessToken");
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?&access_token=${accessToken}`;
    return this.http.delete<InvAgentModel>(url_api, id).pipe(map(data => data));
  }

  getRecordById(id) {
    const accessToken = localStorage.getItem("accessToken");
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?access_token=${accessToken}`;
    return this.http.get<InvAgentModel>(url_api, id).pipe(map(data => data));
  }
