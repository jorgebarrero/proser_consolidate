...
export class AuxIntervalService {
  ...
  getAllRecords(query?): Observable<AuxIntervalModel[]> {
    ...
    return this.http.get<AuxIntervalModel[]>(url_api, {headers: this.headers})
    .pipe(
      map(data => data),
  ...
  postRecord(item?) {
    ...
    return this.http.post<AuxIntervalModel>(url_api, query, {headers: this.headers})
    .pipe(map(data => data));
  }
  ...
  putRecord(item?) {
    ...
    return this.http.put<AuxIntervalModel>(url_api, query, {headers: this.headers})
    .pipe(map(data => data));
  }
  ...
  deleteRecord(id) {
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?&access_token=${accessToken}`;
    return this.http.delete<AuxIntervalModel>(url_api, id)
    .pipe(map(data => data));
  }
  ...
  getRecordById(id) {
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?access_token=${accessToken}`;
    return this.http.get<AuxIntervalModel>(url_api, id)
    .pipe(map(data => data));
  }
