...
import { HcaAgentService, ExcelService } from 'shared/services/';
...
export class CrudHcaAgentComponent implements OnInit {
  ...
  onExportToExcel(data, name) {
    ...
    const filterData = data.map(x => {
      return {
        id: x.hca_agent_id,
        nombre: x.hca_agent_name,
        estatus: x.hca_agent_status,
        extension: x.hca_agent_extension,
        cedula: x.hca_agent_legal_id,
        id_interno: x.hca_agent_internal_id,
        supervisor: x.hca_agent_supervisor_json_name,
        horario: x.hca_agent_schedule_json_name,
        asignacion: x.hca_agent_assignation,
        rol: x.hca_agent_role_name,
      };
    });
