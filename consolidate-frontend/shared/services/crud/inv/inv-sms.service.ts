...
import { InvSmsModel } from 'shared/models';


@Injectable({
  providedIn: 'root'
})
export class InvSmsService {


  api_string = '/api/InvSms';
  ...
  getAllRecords(query?): Observable<InvSmsModel[]> {
    ...
    return this.http.get<InvSmsModel[]>(url_api, {headers: this.headers})
    .pipe(
      map(data => data),
  ...
  postRecord(item?) {
    ...
    return this.http.post<InvSmsModel>(url_api, query, {headers: this.headers})
    .pipe(map(data => data));
  }
  ...
  putRecord(item?) {
    ...
    return this.http.put<InvSmsModel>(url_api, query, {headers: this.headers})
    .pipe(map(data => data));
  }
  ...
  deleteRecord(id) {
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?&access_token=${accessToken}`;
    return this.http.delete<InvSmsModel>(url_api, id)
    .pipe(map(data => data));
  }
  ...
  getRecordById(id) {
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?access_token=${accessToken}`;
    return this.http.get<InvSmsModel>(url_api, id)
    .pipe(map(data => data));
  }
