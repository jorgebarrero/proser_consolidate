...
import { InvQueueModel } from 'shared/models';

@Injectable({
  providedIn: 'root',
  })
export class InvQueueService {
  api_string = '/api/InvQueues'
  ...
  getAllRecords(query?): Observable<InvQueueModel[]> {
    ...
    return this.http
      .get<InvQueueModel[]>(url_api, { headers: this.headers })
      .pipe(
        map(data => data),
  ...
  postRecord(item?) {
    ...
    return this.http
      .post<InvQueueModel>(url_api, query, { headers: this.headers })
      .pipe(map(data => data));
  }
  ...
  putRecord(item?) {
    ...
    return this.http
      .put<InvQueueModel>(url_api, query, { headers: this.headers })
      .pipe(map(data => data));
  }
  ...
  deleteRecord(id) {
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?&access_token=${accessToken}`;
    return this.http.delete<InvQueueModel>(url_api, id).pipe(map(data => data));
  }

  getRecordById(id) {
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?access_token=${accessToken}`;
    return this.http.get<InvQueueModel>(url_api, id).pipe(map(data => data));
  }

  /************************* */

  getScales(query?): Observable<any> {
    const accessToken = localStorage.getItem('accessToken');
    let filter;
    query = `{"where":{"inv_scale_status":"A"}}`;
    if (query) {
      filter = `?filter=${query}&access_token=${accessToken}`;
    } else {
      filter = '?access_token=${accessToken}';
    }
    const url_api = `${this.env.loopbackApiUrl}/api/InvScales${filter}`;
