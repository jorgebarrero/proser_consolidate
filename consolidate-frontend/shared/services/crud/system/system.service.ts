...
export class SystemService {
  ...
  getAllRecords(query?): Observable<SystemModel[]> {
    ...
    const url_api = `${this.env.systemApiUrl}${this.api_string}${filter}`;
    return this.http
  ...
  postRecord(item?) {
    ...
    const url_api = `${this.env.systemApiUrl}${this.api_string}?access_token=${accessToken}`;
    return this.http
  ...
  putRecord(item?) {
    ...
    const query = JSON.stringify(item);
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.systemApiUrl}${this.api_string}/${id}?access_token=${accessToken}`;
  ...
  executePm2(item?): Observable<any> {
    const accessToken = localStorage.getItem('accessToken');
    const query = JSON.stringify(item);
    const url_api = `${this.env.systemApiUrl}${this.api_string}/pm2?access_token=${accessToken}`;
    console.log('url_api', url_api, 'query', query);
