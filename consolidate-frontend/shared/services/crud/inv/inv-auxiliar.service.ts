...
import { InvAuxiliarModel } from 'shared/models';


@Injectable({
  providedIn: 'root'
})
export class InvAuxiliarService {
  ...
  getAllRecords(query?): Observable<InvAuxiliarModel[]> {
    ...
    return this.http.get<InvAuxiliarModel[]>(url_api, {headers: this.headers})
    .pipe(
      map(data => data),
  ...
  postRecord(item?) {
    ...
    return this.http.post<InvAuxiliarModel>(url_api, query, {headers: this.headers})
    .pipe(map(data => data));
  }
  ...
  putRecord(item?) {
    ...
    return this.http.put<InvAuxiliarModel>(url_api, query, {headers: this.headers})
    .pipe(map(data => data));
  }
  ...
  deleteRecord(id) {
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?&access_token=${accessToken}`;
    return this.http.delete<InvAuxiliarModel>(url_api, id)
    .pipe(map(data => data));
  }
  ...
  getRecordById(id) {
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?access_token=${accessToken}`;
    return this.http.get<InvAuxiliarModel>(url_api, id)
    .pipe(map(data => data));
  }
