...
function totalFields() {
  ...
  ,SUM(inboundAttended)/SUM(inboundReceived) AS inboundAtentionLevel
...
function detailFields() {
  ...
  ,SUM(case when callentry_status = 'terminada' then 1 else 0 end)/
   SUM(case when (callentry_status = 'abandonada' OR callentry_status = 'terminada' )then 1 else 0 end) AS inboundAtentionLevel
