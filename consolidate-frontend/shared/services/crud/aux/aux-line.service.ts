...
export class AuxLineService {
  ...
  getAllRecords(query?): Observable<AuxLineModel[]> {
    ...
    return this.http.get<AuxLineModel[]>(url_api, {headers: this.headers})
    .pipe(
      map(data => data),
  ...
  postRecord(item?) {
    ...
    return this.http.post<AuxLineModel>(url_api, query, {headers: this.headers})
    .pipe(map(data => data));
  }
  ...
  putRecord(item?) {
    ...
    return this.http.put<AuxLineModel>(url_api, query, {headers: this.headers})
    .pipe(map(data => data));
  }
  ...
  deleteRecord(id) {
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?&access_token=${accessToken}`;
    return this.http.delete<AuxLineModel>(url_api, id)
    .pipe(map(data => data));
  }
  ...
  getRecordById(id) {
    const accessToken = localStorage.getItem('accessToken');
    const url_api = `${this.env.loopbackApiUrl}${this.api_string}/${id}?access_token=${accessToken}`;
    return this.http.get<AuxLineModel>(url_api, id)
    .pipe(map(data => data));
  }
